...
for image in filtered_images:
    ...
    try:
        ...
        for fuzzer in fuzzers:
            ...
            replace_cmd = f"sed -i 's|--trace=none|{fuzzer}Corpus --trace=none -max_total_time=60 --coverage_report={fuzzer}CovReport|' {filepath}"
            ...
            execute_cmd = f"cd /out/ && mkdir {fuzzer}Corpus && ./{fuzzer} > {fuzzer}Output 2>&1"
            run_cmd(["docker", "exec", container_id, "bash", "-c", execute_cmd])
    
    finally:
        # Copy outputs and reports from the container to the local directory
        for fuzzer in fuzzers:
            # Ensure the target directory exists
            local_dir = f"../containerOut/{fuzzer}Results"
